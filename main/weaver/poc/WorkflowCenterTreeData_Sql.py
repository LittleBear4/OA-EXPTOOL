...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    console.print(now_time() + " [INFO]     正在检测WorkflowCenterTreeData_Sql漏洞", style='bold blue')
    ...
    try:
        ...
        if '[' in res.text:
            console.print(now_time() + " [SUCCESS]     目标为oracle数据库, 可利用sqlmap进行进一步利用: {}".format(target), style='bold green')
        else:
            console.print(now_time() + " [WARNING]  不存在泛微OA WorkflowCenterTreeData接口SQL注入", style='bold red')
    except:
