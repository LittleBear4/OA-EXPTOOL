...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测万户OA DownloadServlet 任意文件读取漏洞", style='bold blue')
    try:
        ...
        if respones.status_code == 200 and 'database' in respones.text:
            console.print(now_time() + ' [SUCCESS]  万户OA DownloadServlet 任意文件读取漏洞存在{}'.format(exp_url), style='bold green')
        else:
            console.print(now_time() + ' [WARNING]  万户OA DownloadServlet 任意文件读取漏洞不存在', style='bold red ')
    except:
