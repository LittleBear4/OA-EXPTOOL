...
def main(target_url):
    ...
    console.print(now_time() + " [INFO]     正在检测致远OA A6 initDataAssess.jsp 用户敏感信息泄露漏洞", style='bold blue')
    ...
    try:
        ...
        if "/yyoa/index.jsp" not in response.text and "personList" in response.text and response.status_code == 200:
            console.print(
                now_time() + " [SUCCESS]  目标存在致远OA A6 initDataAssess.jsp 用户敏感信息泄露漏洞, 泄露地址: {}".format(vuln_url),
                style='bold green')
        else:
            console.print(now_time() + " [WARNING]  致远OA A6 initDataAssess.jsp 用户敏感信息泄露漏洞利用失败", style='bold red')
    except:
